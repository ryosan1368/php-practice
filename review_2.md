...
- forは、繰り返す回数が決まっている場合に使います。foreachは、配列・連想配列のデータに対して、処理を繰り返したい時に使います。
...
- なぜ必要かというと、最初に行っておきたい処理を設定しておくと、インスタンス化した際に処理が実行されるためです。
...
- まず、このコードはDateTimeクラスを利用していることと、３行目のコードはメソッドチェインになっており、formatメソッドは直前に書かれているdiffメソッドの返り値から呼び出されていることに注目します。
...
- diffメソッドの返り値のデータ型を調べると、DateIntervalとあります。DateIntervalのページを開くと、メソッドが書かれており、その中にformatメソッドがあります。
